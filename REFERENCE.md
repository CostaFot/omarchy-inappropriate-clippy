...
- `ClippyMenu.qml` — the menu (right-click on him or the grave, or the bar
  ...
  row's rule), and the two volume rows (v1.50.0, quiet 0.25 · medium
  0.5 · full 1, nearest-chip): "Sound volume" under the Sounds toggle,
  shown only while `slapSoundOn`, and "Voice volume" under the Voice
  picker, shown only while `ttsOn && voiceVolumeApplies` — the
  dead-row rule both times; ●/○ `Entry` toggles for Sounds, "Full-screen gags",
...
- `BarWidget.qml` — the bar icon, `kind: "bar-widget"` on the same
  ...
  `󰏢`). One per monitor. No state of its own, and it shows none: it used
  to fade to 45% while he was dead or hidden, through the panel handle,
  and when 4.0.3 took the handle away nobody missed it, so the fade went
  (COS-157) rather than a poll to bring it back. On omarchy ≤ 4.0.2 it
  finds the panel instance through
  ...
  is null there and the click goes
  ...
  icon on the clicked monitor. No IpcHandler here — the panel owns the
  target.
...
- Flinging: `dragTo` keeps a smoothed pointer speed in px/ms from the
  ...
  defaults to `slapSoundOn` so the menu's "Sounds" row mutes both. All
  three banks play at `soundVolume` (v1.50.0, 0-1, default 1, the
  NaN-tolerant reader) — one `volume:` binding on the SoundBank
  delegate, the SoundEffect's own linear gain, so it never touches
  pactl and the duck script (which skips quickshell's streams anyway)
  never sees it; off stays an empty bank, never volume 0, so the
  slap-then-speak handshake on `playing` is unchanged. IPC
...
- Clones: chatterbox on CUDA in a uv venv (`--python 3.12` — system
  ...
  property (`voiceEnv`, which also carries `CLIPPY_VOICE_VOLUME` — below)
  on ttsProc and both warm Processes (whichever spawns the daemon
...
- `voiceVolume` (0-1, default 1, v1.50.0 — Costa: "the sounds are a bit
  too loud … same for the voice"): his own level. Every engine plays
  through a different pipe, so it reaches only the two the plugin can
  bend without rewriting a stored tts string: the built-in espeak gets
  `-a <0-200>` appended in `launchTts()`, and speak-clone reads
  `CLIPPY_VOICE_VOLUME` from `voiceEnv` and applies `volume=` in the
  same ffmpeg step as tempo/pitch, into its own `-v<V>` derivative
  (the raw cache key is untouched, so warm-voice and every existing
  render stay valid). An env var, NOT a flag, on purpose: a client
  installed before the knob ignores an unknown variable, where an
  unknown flag would argparse-exit into silence — so there is no
  old-client probe (setup-voice is already the fragile script; the
  contact is four lines in the heredoc and the set reply says "a clone
  installed before v1.50.0 needs one setup-voice rerun"). Robot George,
  piper and custom commands are frozen strings and ignore it — George's
  ffmpeg chain COULD read the variable, but changing the string breaks
  `currentVoiceId`'s exact match on every existing install; Costa's
  call was espeak + clones only, the rest answer "ok — but". The
  `voiceVolumeApplies` gate (`ttsSetting === true || cloneKnobsApply`)
  hides the menu row for those engines.
...
- The host surface is not stable across omarchy releases, and when it
  ...
  (the bar icon's handle on the panel — the `showMenuAt` verb stands in),
